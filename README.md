...
- Combine을 사용하여 이벤트 스트림 관리 및 UI와 데이터 간의 반응형 바인딩을 구현했습니다.
...
[WWDC21: Demystify SwiftUI](https://developer.apple.com/videos/play/wwdc2021/10022/)에 의하면 SwiftUI는 뷰를 렌더링할 때 저장된 View Value의 사본을 기반으로 diffing을 수행합니다.
이때 View Value 사본에서 이미 삭제된 객체의 메모리 주소를 참조하게 되어 충돌 문제가 발생했습니다.
...
고유한 식별 값을 가지고 있는 객체 내 id 속성을 사용하도록 변경하여 해결했습니다.

이렇게 하면 diffing 과정에서 객체의 메모리 주소 대신 `id` 값을 기준으로 비교하게 되며, 결과적으로 삭제된 객체의 메모리 주소를 참조하는 일이 없어지기 때문에 충돌 문제가 발생하지 않도록 보장됩니다.
